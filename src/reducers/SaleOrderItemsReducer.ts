...
const SaleOrderItemsReducer = (
  state: ContextState,
  action: SaleOrderItemsActions
): ContextState => {
  switch (action.type) {
    ...
    case 'DELETE_SALE_ORDER_ITEM':
      return {
        ...state,
        saleOrderItems: state.saleOrderItems.filter(
          (saleOrderItem) => saleOrderItem.id !== action.payload
        )
      };

    case 'EDIT_SALE_ORDER_ITEM':
      const found = state.saleOrderItems.find(
